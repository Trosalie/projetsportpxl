...
export class SubscriptionForm implements OnDestroy {
  ...
  onSubmit(event: Event) {
    ...
    const priceHT = form['priceHT'].value;
    ...
    if (!subject || !startDate || !endDate || !priceHT || !description || !TVA || !this.findClient) {
    ...
    this.modalData = {
      title: subject,
      amount: parseFloat(priceHT),
      items: [
        { label: 'Photographe', value: this.photographerInput },
        { label: 'Abonnement', value: `${description}€` },
  ...
  onConfirmInvoice() {
    ...
    const { startDate, endDate, subject, priceHT, description, TVA, dueDate } = this.pendingFormData;
    const body = {
      labelTVA: TVA,
      amountEuro: priceHT,
      ...
      invoiceDescription: `${description}€ pour la période du ${startDate} au ${endDate}.`
    }
    this.creationFacture = true;
    this.showConfirmModal = false;
    this.invoiceService.createSubscriptionInvoice(body)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
          ...
          this.modalData = null;
          this.insertSubscriptionInvoice(response, priceHT, startDate, endDate, description, TVA, this.today, dueDate, this.clientId, "À venir");
          setTimeout(() => {
  ...
  insertSubscriptionInvoice(reponse: any, amount: number, startDate: string, endDate: string, description: string, tva: string, issueDate: string, dueDate: string, clientId: number, status: string) {
    ...
    this.invoiceService.insertSubscriptionInvoice(body)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {},
